...
"""
function play(input, segment; volume_scale=0.8, rate=200)
    seg = string(input[segment])
    @debug "$(volume_scale == 0 ? "🎤Pausing" : "👂Playing"): $seg"
    run(`say \[\[volm $(volume_scale)\]\] \[\[rate $rate\]\] $seg`)
    return nothing
end
...
function get_learnspan(i_segment, segments; iteration_mode)
    if iteration_mode == :cumulative
        ...
        return length(current_segments) == 1 ? only(current_segments) :
               vcat(current_segments)
    end
...
function learn_demo(; num_repetitions=2, iteration_mode=:sequential,
    ...
    return learn(DEMO_ALPHABET, segments; num_repetitions, iteration_mode,
                 interrepeat_pause)
end
